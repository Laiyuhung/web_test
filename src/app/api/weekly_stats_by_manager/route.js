...
export async function POST(req) {
  try {
    ...
    if (isPastWeek) {
      ...
      if (under30.length > 0) {
        const pitcherStats = Object.keys(result[0].pitchers.fantasyPoints || {});
        // 對每個投手項目分別處理
        pitcherStats.forEach(stat => {
          // 取所有隊伍該項分數，排序（由小到大）
          const allScores = result.map(r => r.pitchers.fantasyPoints[stat] ?? Infinity).sort((a, b) => a - b);
          // 取違規隊伍數量
          const n = under30.length;
          // 取最低n名分數
          const lowestN = allScores.slice(0, n);
          // 平均分配給違規隊伍
          const avg = lowestN.reduce((a, b) => a + b, 0) / n;
          result.forEach(r => {
            if (under30.some(t => t.team === r.team_name)) {
              r.pitchers.fantasyPoints[stat] = parseFloat(avg.toFixed(1));
            }
          });
        });
